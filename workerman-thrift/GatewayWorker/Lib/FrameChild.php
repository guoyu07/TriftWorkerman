...
class FrameChild extends EventDispatcher {
	...
	protected $frameStopTime = 0;
	protected static $frameUseTimeStack = array();
	/**
	 * 帧循环
	 */
	public function __onEnterFrame() {
		$this->frameStartTime = $this->getMillisecond();
		...
		if ($this->readyDestory) { // 准备销毁了
			...
			$this->onDestory ();
			$this->__reset ();
			$this->frameStopTime = $this->getMillisecond();
			return false;
	...
	public function getUseTime(){
		return $this->frameStopTime-$this->frameStartTime;
	}
	/**
	 * 获取帧处理时间堆栈
	 */
	public function getFrameUseTimeStack(){
		self::$frameUseTimeStack = array();
		foreach ( $this->children_list as $value ) {
			$value->getFrameUseTimeStack();
		}		
		array_push(self::$frameUseTimeStack, '[class:'.get_called_class().' key:'.$this->key.']:'.$this->getUseTime()."ms");
		return self::$frameUseTimeStack;
	}
