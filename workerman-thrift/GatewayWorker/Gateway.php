...
class Gateway extends Worker {
	...
	public function onWorkerStart() {
		...
		if ($this->pingInterval > 0) {
			...
			Timer::add ( $timer_interval, array (
					$this,
					'ping' 
			) );
		}
		...
		if ($this->lanIp !== '127.0.0.1') {
			Timer::add ( self::PERSISTENCE_CONNECTION_PING_INTERVAL, array (
					$this,
					'pingBusinessWorker' 
			) );
		}
		...
		// 设置内部监听的相关回调
		$this->_innerTcpWorker->onMessage = array (
				$this,
				'onWorkerMessage' 
		);
		
		$this->_innerTcpWorker->onConnect = array (
				$this,
				'onWorkerConnect' 
		);
		$this->_innerTcpWorker->onClose = array (
				$this,
				'onWorkerClose' 
		);
	...
	public function registerAddress() {
		...
		$this->_registerConnection->onClose = array (
				$this,
				'onRegisterConnectionClose' 
		);
